...
single_scenario_simulation_tuning <- function(start_n_predictors, start_prevs, start_shrinkages, N_train, n_valid, betas_matrix, betas_matrix_valid, shrinkages, prevs, n_predictors, start_simulation, end_simulation){
  ...
  if (begin_simulation_flag > 0) {
    repeat {
      file_name_recovery_bk <- paste(paste(path_name, "data/sim_results/simulation_data", sep = "/"), "i_simulation", i_simulation, "shrinkage", start_shrinkages, "prevalence", start_prevs, "n_predictors", start_n_predictors, "seed_valid", seed_valid, "seed_train", seed_train, "batch", batch_number, "s.RData", sep="_")
